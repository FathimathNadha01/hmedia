...
export default function FullScreenAdFormPopup({ isOpen, onClose, onSubmit, ad }) {
  const [formData, setFormData] = useState({
    title: "",
    pageType: PAGE_TYPE_OPTIONS[0],
    link: "",
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div className="bg-gray-800 w-full max-w-4xl rounded-lg border border-gray-700 shadow-sm text-white max-h-[90vh] overflow-y-auto scrollbar-hide">
        ...
        <form id="fullscreen-ad-form" onSubmit={handleSubmit} className="p-6 space-y-6">
          {/* Image Upload */}
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Ad Image</label>
            <label htmlFor="file-upload" className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-600 border-dashed rounded-md cursor-pointer">
              <div className="space-y-1 text-center">
                {imagePreview ? (
                  <div className="relative inline-block">
                    <img src={imagePreview} alt="Preview" className="mx-auto h-40 rounded-md object-contain" />
                    <button type="button" onClick={handleRemoveImage} className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full p-1 shadow-md hover:bg-red-700 transition-colors">
                      <X size={16} />
                    </button>
                  </div>
                ) : (
                  <>
                    <UploadCloud className="mx-auto h-12 w-12 text-gray-500" />
                    <p className="text-sm text-gray-400">Click to upload</p>
                    <p className="text-xs text-gray-500">Recommended: Portrait/Landscape</p>
                  </>
                )}
              </div>
            </label>
          ...
          <div>
            ...
            <input type="text" id="title" name="title" value={formData.title} onChange={handleInputChange} placeholder="Enter Title"
              className="mt-1 block w-full bg-gray-900 border border-gray-600 focus:outline-none focus:ring-0 focus:border-brand-red rounded-lg py-2.5 px-3 text-sm" />
          ...
          {/* Page Type */}
          <div className="relative w-full">
            <label htmlFor="pageType" className="block text-sm font-medium text-gray-300 mb-1">Page Type</label>
            <button type="button" className="w-full sm:w-60 bg-gray-900 border border-gray-600 rounded-lg pl-4 pr-10 py-2 text-left flex justify-between items-center focus:outline-none focus:ring-2 focus:ring-brand-red transition-all cursor-pointer" onClick={() => setDropdownOpen(!dropdownOpen)}>
              <span>{formData.pageType}</span>
            ...
            {dropdownOpen && (
              <ul className="absolute z-50 mt-1 sm:w-60 w-full bg-gray-900 border border-gray-600 rounded-lg shadow-lg max-h-60 overflow-auto scrollbar-hide">
                {PAGE_TYPE_OPTIONS.map((opt) => (
                  <li key={opt} className={`px-4 py-2 cursor-pointer text-gray-300 hover:bg-brand-red hover:text-white transition-colors ${formData.pageType === opt ? "bg-brand-red text-white" : ""}`} onClick={() => { setFormData(prev => ({ ...prev, pageType: opt })); setDropdownOpen(false); }}>
                    {opt}
                  </li>
                ))}
              </ul>
          ...
          <div>
            ...
            <input type="text" id="link" name="link" value={formData.link} onChange={handleInputChange} placeholder="https://example.com"
              className="mt-1 block w-full bg-gray-900 border border-gray-600 focus:outline-none focus:ring-0 focus:border-brand-red rounded-lg py-2.5 px-3 text-sm" />
          </div>
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => setFormData(prev => ({ ...prev, status: prev.status === "Active" ? "Inactive" : "Active" }))} className={`${formData.status === "Active" ? "bg-green-500" : "bg-gray-600"} h-6 w-11 rounded-full transition cursor-pointer`} role="switch" aria-checked={formData.status === "Active"}>
                <span className={`${formData.status === "Active" ? "translate-x-3" : "-translate-x-3"} inline-block h-6 w-6 bg-white rounded-full transform transition`} />
              </button>
              <span className="text-sm font-medium text-gray-300 w-14 text-right">{formData.status}</span>
            </div>
          </div>
        </form>
        ...
        {/* Footer */}
        <div className="sticky bottom-0 bg-gray-800 flex justify-end items-center gap-4 p-5 border-t border-gray-700">
          <button onClick={onClose} disabled={isSubmitting} className="px-4 py-2 rounded-lg text-gray-300 hover:bg-gray-700 transition-colors text-sm font-medium cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">Cancel</button>
          <button type="submit" form="fullscreen-ad-form" disabled={isSubmitting} className="px-6 py-2 rounded-lg bg-brand-red text-white font-semibold hover:bg-brand-dark transition-colors text-sm cursor-pointer flex items-center justify-center disabled:bg-brand-dark disabled:cursor-wait">
            {isSubmitting ? "Processing..." : ad ? "Save Changes" : "Add Full Screen Ad"}
          </button>
        </div>
      </div>
    </div>
