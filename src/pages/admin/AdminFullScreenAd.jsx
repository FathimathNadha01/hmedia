...
function AdminFullScreenAd() {
  ...
  const loadAds = async () => {
    try {
      ...
      const data = await fetchFullScreenAds(baseURL); // fetch from backend
  ...
  const handleFormSubmit = async (formData) => {
    try {
      setIsProcessing(true);
      if (editingAd) {
        await updateFullScreenAd(baseURL, editingAd.id, formData);
      } else {
        await createFullScreenAd(baseURL, formData);
      }
      await loadAds();
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">Full Screen Advertisement</h1>
          <p className="text-gray-700 mt-1 text-sm md:text-base">Manage full screen advertisements.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
          ...
          >
            <Plus size={20} />
            {isProcessing ? "Processing..." : "Add Full Screen Ad"}
          </button>
        </div>
      </div>
